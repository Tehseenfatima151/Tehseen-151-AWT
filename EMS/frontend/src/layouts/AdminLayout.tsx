...
import {
  LayoutDashboard, ClipboardList, CheckCircle2, XCircle, Zap, Trophy,
  Users, UserCheck, UserX, Shield, AlertTriangle, FileText, Bell,
  BarChart3, Activity, Database, Settings, User, LogOut, Menu, X,
  ChevronDown, ChevronRight, Vote, Sun, Moon
} from 'lucide-react';
...
const adminNav: NavItem[] = [
  ...
  {
    label: 'Elections', icon: Vote, children: [
      { label: 'Pending Requests', href: '/admin/elections/requests', icon: ClipboardList },
      { label: 'Approved', href: '/admin/elections/approved', icon: CheckCircle2 },
      { label: 'Rejected', href: '/admin/elections/rejected', icon: XCircle },
      { label: 'Active', href: '/admin/elections/active', icon: Zap },
      { label: 'Completed', href: '/admin/elections/completed', icon: Trophy },
    ]
  },
  {
    label: 'Users', icon: Users, children: [
      { label: 'All Users', href: '/admin/users', icon: Users },
      { label: 'Creators', href: '/admin/users/creators', icon: UserCheck },
      { label: 'Voters', href: '/admin/users/voters', icon: User },
      { label: 'Blocked', href: '/admin/users/blocked', icon: UserX },
    ]
  },
  {
    label: 'Security', icon: Shield, children: [
      { label: 'Security Center', href: '/admin/security', icon: Shield },
      { label: 'Fraud Detection', href: '/admin/security/fraud', icon: AlertTriangle },
  ...
  { label: 'Reports', icon: BarChart3, href: '/admin/reports' },
  {
    label: 'System', icon: Activity, children: [
      { label: 'API Monitoring', href: '/admin/api-monitoring', icon: Activity },
      { label: 'DB Health', href: '/admin/database-health', icon: Database },
    ]
  },
...
function NavLink({ item, collapsed }: { item: NavItem; collapsed: boolean }) {
  ...
  const isActive = item.href ? location.pathname === item.href : item.children?.some(c => location.pathname === c.href);
      ...
      <div className="flex items-center gap-3 px-5 py-6 bg-gradient-to-b from-primary/5 to-transparent">
        ...
        {!collapsed && (
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">Admin Portal</span>
          </div>
        )}
      </div>

      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        {adminNav.map(item => <NavLink key={item.label} item={item} collapsed={collapsed} />)}
      </nav>
    </div>
  );
    ...
    <div className="flex h-screen bg-background overflow-hidden">
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }}
              ...
              className="fixed left-0 top-0 h-full w-72 bg-white dark:bg-slate-900 shadow-2xl z-50 md:hidden">
              <SidebarContent />
            </motion.aside>
          </>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-16 bg-card/80 backdrop-blur-xl flex items-center px-4 sm:px-6 gap-4 shrink-0 relative z-30 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
          ...
          <Link to="/admin/notifications" className="relative p-2 rounded-lg hover:bg-muted">
            <Bell size={18} />
          ...
          <div className="relative">
            <button onClick={() => setProfileDropdownOpen(!profileDropdownOpen)} className="flex items-center gap-2.5 hover:bg-muted/50 p-1 pl-3 rounded-full transition-colors text-right border border-transparent hover:border-border">
              <div className="hidden sm:block">
                ...
                <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide leading-none">Super Admin</p>
              </div>
                ...
                <>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                    ...
                    <div className="px-4 py-3 border-b border-border bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold mt-0.5">Super Admin</p>
                    </div>
                    <Link to="/admin/profile" onClick={() => setProfileDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold hover:bg-muted"><User size={16} /> My Profile</Link>
                    <Link to="/admin/settings" onClick={() => setProfileDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold hover:bg-muted"><Settings size={16} /> Settings</Link>
        ...
        <main className="flex-1 overflow-y-auto">
          <AnimatePresence mode="wait">
            <motion.div key={useLocation().pathname} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="p-4 md:p-6 lg:p-8 min-h-full">
              <Outlet />
            </motion.div>
          </AnimatePresence>
        </main>
