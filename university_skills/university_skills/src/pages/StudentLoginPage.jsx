...
export default function StudentLoginPage() {
  ...
  useEffect(() => {
    if (location.state?.reason === 'no_profile') {
      toast.error('Profile row missing or blocked. Run SQL fix in README / fix_rls_recursion.sql, or add your row in public.users.')
    }
  }, [location.state])
        ...
        <div className="rounded-[22px] bg-white p-6 md:p-8">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h1 className="text-2xl font-bold text-slate-900">{showForgot ? 'Reset password' : 'Student Login'}</h1>
              <p className="mt-1 text-sm leading-relaxed text-slate-600">
                {showForgot
                  ? 'Enter your email and we will send you a link to choose a new password.'
                  : 'Sign in to manage your skills, portfolio, and university credentials.'}
              </p>
            </div>
          ...
          {showForgot ? (
            <div className="mt-5 space-y-4">
              <div>
                <label htmlFor="student-reset-email" className="mb-1 block text-sm font-medium text-slate-700">
                  Email address
                </label>
                <input
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-900/15 transition hover:bg-sky-500 disabled:opacity-60"
              >
            ...
            <>
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <div className="mb-1 flex items-center justify-between gap-2">
                    ...
                    <button
                      type="button"
                      className="shrink-0 text-sm font-semibold text-sky-700 underline-offset-2 hover:text-sky-800 hover:underline"
                      onClick={() => {
                        setShowForgot(true)
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-900/15 transition hover:bg-sky-500 disabled:opacity-60"
              >
                {loading ? (
