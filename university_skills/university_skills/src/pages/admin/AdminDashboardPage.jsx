...
import { getCachedAdminDashboard, setCachedAdminDashboard, subscribeToCache } from '../../utils/dashboardCache'
...
export default function AdminDashboardPage() {
  ...
  const cachedData = getCachedAdminDashboard()

  const [stats, setStats] = useState(cachedData ? cachedData.stats : {
    students: 0,
    skills: 0,
    projects: 0,
    certificates: 0,
  ...
  const fetchDashboardData = useCallback(() => {
    ...
    Promise.all([
    ...
    ]).then(([statsVal, notificationsVal]) => {
      const freshData = {
        stats: statsVal,
      ...
      setCachedAdminDashboard(freshData)
    }).finally(() => {
      setLoading(false)
    })
  }, [profile, cachedData])
  ...
  useEffect(() => {
    ...
    const channel = subscribeToNotifications(profile.id, (newNotif) => {
      setNotifications((prev) => {
        ...
        const currentCache = getCachedAdminDashboard()
        if (currentCache) {
          currentCache.notifications = next
          setCachedAdminDashboard(currentCache)
        }
        return next
      })
      fetchDashboardData()
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-3 md:grid-cols-4 md:gap-4 lg:grid-cols-4 xl:grid-cols-4">
        {loading ? (
          <>
            <div className="h-[92px] animate-pulse rounded-xl bg-white/5" />
            <div className="h-[92px] animate-pulse rounded-xl bg-white/5" />
            <div className="h-[92px] animate-pulse rounded-xl bg-white/5" />
            <div className="h-[92px] animate-pulse rounded-xl bg-white/5" />
            <div className="h-[92px] animate-pulse rounded-xl bg-white/5" />
